...
|_______/    |_______| \______||_______/ | _| `._____| \______/  |__| |_______/ 
    ...
    logging.info("[+] SecDroid - a comprehensive static code analysis tool for Android apps")
    logging.info("[+] Based on: OWASP MASVS - https://mobile-security.gitbook.io/masvs/")
    logging.info("[+] Author - Shoaib Attar")
    logging.info("[*] Connect: Feel free to give feedback.")
...
def SecDroid_help():
    ...
    print("\t -l     Enable logging (outputs to a .txt file)")
...
def SecDroid_core_log(apkpath):
    logging.basicConfig(format="%(message)s", level=logging.INFO)
    # Get current timestamp
    ctime = time.strftime("%Y-%m-%d_%H-%M-%S")

    # Extract APK filename without extension
    apk_file_name = os.path.splitext(os.path.basename(apkpath))[0]

    # Define log and HTML file paths
    log_file_path = os.path.join(os.path.dirname(apkpath), f"SecDroid_{apk_file_name}_{ctime}.txt")
    html_file_path = os.path.join(os.path.dirname(apkpath), f"SecDroid_{apk_file_name}_{ctime}.html")

    # Setup logging to both console and log file
    log_file = open(log_file_path, "w")
    html_file = open(html_file_path, "w")

    logging.basicConfig(level=logging.INFO, format="%(message)s", handlers=[logging.StreamHandler(), logging.FileHandler(log_file)])

    # Write HTML report header
    html_file.write("<html><head><title>SecDroid Report</title></head><body>\n")
    html_file.write("<h1>SecDroid Report</h1>\n<pre>\n")

    logging.info("\n[+] Log-file path: " + log_file_path)
    logging.info("\n[+] HTML-file path: " + html_file_path)

    # Close log and HTML files properly
...
def SecDroid_core(apkpath):
    ...
    logging.info(f"[+] APK Base: {apkpathbase}")
    ...
    logging.info(f"[+] APK Directory: {apkpathdir}")
    ...
    if not re.match(r'^[a-zA-Z0-9_-]*$', apkname):
        logging.error("[!] Only Alphanumeric string with/without underscore/dash is accepted as APK file-name. Please rename the APK file.")
        exit(0)
    ...
    logging.info(f"[+] APK Hash: MD5: {md5_hash}")
    logging.info(f"[+] APK Hash: SHA256: {sha256_hash}")
    ...
    try:
        ...
        logging.info(cmd_apk_dex2jar.stdout)
        if cmd_apk_dex2jar.stderr:
            logging.error(cmd_apk_dex2jar.stderr)
    except Exception as e:
        logging.error(f"Error running d2j-dex2jar: {e}")
    
    logging.info(f"[+] Jadx has started decompiling the application")
    logging.info("[+] ============================================")
    try:
        # Run Jadx
        ...
        logging.info(cmd_apk_jadx.stdout)
        if cmd_apk_jadx.stderr:
            logging.error(cmd_apk_jadx.stderr)
    except Exception as e:
        logging.error(f"Error running Jadx: {e}")
    ...
    try:
        ...
        if package_name:
            logging.info(f"   {package_name.group()}")
        else:
            logging.warning("    - Package Name has not been observed.")
    ...
    try:
        ...
        if version_name:
            logging.info(f"   {version_name.group()}")
        else:
            logging.warning("    - android:versionName has not been observed.")
    except subprocess.CalledProcessError:
        logging.warning("    - android:versionName has not been observed.")
    ...
    try:
        ...
        if min_sdk_version:
            logging.info(f"   {min_sdk_version.group()}")
        else:
            logging.warning("    - android:minSdkVersion has not been observed.")
    except subprocess.CalledProcessError:
        logging.warning("    - android:minSdkVersion has not been observed.")
    ...
    try:
        ...
        if target_sdk_version:
            logging.info(f"   {target_sdk_version.group()}")
        else:
            logging.warning("    - android:targetSdkVersion has not been observed.")
    except subprocess.CalledProcessError:
        logging.warning("    - android:targetSdkVersion has not been observed.")
    ...
    try:
        ...
        if nwSecConf_match:
            ...
            logging.info(f"   {nwSecConf_final}")
        else:
            logging.warning("    - android:networkSecurityConfig attribute has not been observed.")
    except subprocess.CalledProcessError:
        logging.warning("    - android:networkSecurityConfig attribute has not been observed.")
    ...
    try:
        ...
        logging.info(cmd_and_actv)
    except subprocess.CalledProcessError:
        logging.warning("- No activities have been observed")

    # Exported Activities
    logging.info("[+] Looking for the Exported Activities specifically...\n\n")
    ...
    try:
        ...
        logging.info(cmd_and_exp_actv)
        exported_count = cmd_and_exp_actv.count('android:exported="true"')
        logging.info(f"    > Total exported activities are: {exported_count}")
        logging.info("\n    > QuickNote: It is recommended to use exported activities securely, if observed.\n")
    except subprocess.CalledProcessError:
        logging.warning("\t- No exported activities have been observed.")
    ...
    try:
        ...
        logging.info(cmd_and_cont)
    except subprocess.CalledProcessError:
        logging.warning("\t- No Content Providers have been observed.")

    # Exported Content Providers
    logging.info("[+] Looking for the Exported Content Providers specifically...\n\n")
    ...
    try:
        ...
        logging.info(cmd_and_exp_cont)
        exported_count = cmd_and_exp_cont.count('android:exported="true"')
        logging.info(f"    > Total exported Content Providers are: {exported_count}")
    except subprocess.CalledProcessError:
        logging.warning("\t- No exported Content Providers have been observed.")
    ...
    try:
        ...
        logging.info(cmd_and_brod)
    except subprocess.CalledProcessError:
        logging.warning("\t- No Broadcast Receivers have been observed.")

    # Exported Broadcast Receivers
    logging.info("[+] Looking for the Exported Broadcast Receivers specifically...\n\n")
    ...
    try:
        ...
        logging.info(cmd_and_exp_brod)
        exported_count = cmd_and_exp_brod.count('android:exported="true"')
        logging.info(f"    > Total exported Broadcast Receivers are: {exported_count}")
    except subprocess.CalledProcessError:
        logging.warning("\t- No exported Broadcast Receivers have been observed.")
    ...
    try:
        ...
        logging.info(cmd_and_serv)
    except subprocess.CalledProcessError:
        logging.warning("\t- No Services have been observed.")

    # Exported Services
    logging.info("[+] Looking for the Exported Services specifically...\n\n")
    ...
    try:
        ...
        logging.info(cmd_and_exp_serv)
        exported_count = cmd_and_exp_serv.count('android:exported="true"')
        logging.info(f"    > Total exported Services are: {exported_count}")
    except subprocess.CalledProcessError:
        logging.warning("\t- No exported Services have been observed.")
    ...
    try:
        ...
        logging.info(cmd_and_intentFilters)
    except subprocess.CalledProcessError:
        logging.warning("\t- No Intent Filters have been observed.")

    # APK Component Summary
    logging.info("\n==>> APK Component Summary")
    logging.info("[+] --------------------------------")
    logging.info(f"    Exported Activities: {exported_count}")
    logging.info(f"    Exported Content Providers: {exported_count}")
    logging.info(f"    Exported Broadcast Receivers: {exported_count}")
    logging.info(f"    Exported Services: {exported_count}")
    ...
    logging.info("\n")
    logging.info("[+] Let's start the static assessment based on 'OWASP MASVS'")
    logging.info("[+] ========================================================")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "getSharedPreferences" in cmd_and_pkg_getSharedPreferences_output:
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_getSharedPreferences_output)
                    countSharedPref += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countSharedPref > 0:
        ...
        logging.info("    - It is recommended to use shared preferences appropriately, if observed. "
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if ("openOrCreateDatabase" in cmd_and_pkg_sqlitedatbase_output or
                    ...
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_sqlitedatbase_output)
                    countSqliteDb += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countSqliteDb > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that sensitive data should not be stored in unencrypted SQLite databases, if observed. "
        ...
        if sources_file.endswith(".xml"):
            try:
                ...
                if "firebaseio" in cmd_and_pkg_firebase_output:
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_firebase_output)
                    countFireDB += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countFireDB > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that Firebase Realtime database instances should not be misconfigured, if observed. "
        ...
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-STORAGE-2 | CWE-200: Exposure of Sensitive Information to an Unauthorized Actor")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2-data_storage_and_privacy_requirements")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "RealmConfiguration" in cmd_and_pkg_realm_output:
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_realm_output)
                    countRealmDB += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countRealmDB > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that Realm database instances should not be misconfigured, if observed. "
        ...
        if sources_file.endswith(".java"):
            ...
            if "MODE_WORLD_READABLE" in cmd_and_pkg_internalStorage_output or "MODE_WORLD_WRITEABLE" in cmd_and_pkg_internalStorage_output:
                ...
                if "openFileOutput" in cmd_and_pkg_internalStorage_output or "FileInputStream" in cmd_and_pkg_internalStorage_output or "MODE_WORLD_READABLE" in cmd_and_pkg_internalStorage_output or "MODE_WORLD_WRITEABLE" in cmd_and_pkg_internalStorage_output:
                    logging.info(cmd_and_pkg_internalStorage_output)
                    countIntStorage += 1

    if countIntStorage > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that sensitive files saved to the internal storage should not be accessed by other applications, if observed. Modes such as MODE_WORLD_READABLE and MODE_WORLD_WRITEABLE may pose a security risk.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-STORAGE-2 | CWE-922: Insecure Storage of Sensitive Information")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2-data_storage_and_privacy_requirements")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if any(x in cmd_and_pkg_externalStorage_output for x in ["getExternalFilesDirs(", "getExternalCacheDir(", "getExternalFilesDirs(", "getCacheDir(", "getExternalStorageState(", "getExternalStorageDirectory(", "getExternalStoragePublicDirectory("]):
                logging.info(sources_file)
                logging.info(cmd_and_pkg_externalStorage_output)
                countExtStorage += 1

    if countExtStorage > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that any sensitive data should not be stored in the external storage, if observed. Files saved to external storage are world-readable and may be accessed by unauthorized parties.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-STORAGE-2 | CWE-922: Insecure Storage of Sensitive Information")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2-data_storage_and_privacy_requirements")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if ".createTempFile(" in cmd_and_pkg_tempFile_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_tempFile_output)
                countTempFile += 1

    if countTempFile > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that the temporary files should be securely deleted upon their usage, if observed. Insecure temporary files can leave application and system data vulnerable to attack.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-STORAGE-2 | CWE-277: Insecure Inherited Permissions")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2-data_storage_and_privacy_requirements")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "SharedPreferences.Editor" in cmd_and_pkg_sharedPreferencesEditor_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_sharedPreferencesEditor_output)
                countSharedPrefEd += 1

    if countSharedPrefEd > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that input validation needs to be applied on sensitive data when it is read back again. Any process can override the data for publicly accessible data storage.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-2 | CWE-922: Insecure Storage of Sensitive Information")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if any(x in cmd_and_pkg_logs_output for x in ["Log.v(", "Log.d(", "Log.i(", "Log.w(", "Log.e(", "logger.log(", "logger.logp(", "log.info", "System.out.print", "System.err.print"]):
                logging.info(sources_file)
                logging.info(cmd_and_pkg_logs_output)
                countLogs += 1
                countLogs2 += cmd_and_pkg_logs_output.count("\n")

    if countLogs > 0:
        logging.info(f"[+] Total file sources are: {countLogs} & its total instances are: {countLogs2}")
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that any sensitive data should not be part of the log's output or revealed in Stacktraces.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-STORAGE-3 | CWE-532: Insertion of Sensitive Information into Log File")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2-data_storage_and_privacy_requirements")
            ...
            try:
                ...
                if "setContentTitle" in cmd_and_pkg_notificationManager_output or "setContentText" in cmd_and_pkg_notificationManager_output:
                    ...
                    if "NotificationManager" in cmd_and_pkg_notificationManager_output or "setContentTitle" in cmd_and_pkg_notificationManager_output or "setContentText" in cmd_and_pkg_notificationManager_output:
                        logging.info(cmd_and_pkg_notificationManager_output)
                        countNotiManag += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countNotiManag > 0:
        ...
        logging.info("[!] QuickNote:")
        print(colorReset)
        logging.info("    - It is recommended that any sensitive data should not be notified via the push notifications, if observed. Please note that, It would be necessary to understand how the application is generating the notifications and which data ends up being shown.")
        print(colorCyan)
        logging.info("\n[*] Reference:")
        print(colorReset)
        logging.info("    - OWASP MASVS: MSTG-STORAGE-4 | CWE-829: Inclusion of Functionality from Untrusted Control Sphere")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2-data_storage_and_privacy_requirements")
        ...
        if sources_file.endswith(".xml"):
            try:
                ...
                if "textAutoComplete" in cmd_and_pkg_keyboardCache_output or "textAutoCorrect" in cmd_and_pkg_keyboardCache_output:
                    ...
                    logging.info(sources_file)
                    print(colorReset)
                    logging.info(cmd_and_pkg_keyboardCache_output)
                    countKeyCache += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countKeyCache > 0:
        ...
        logging.info("[!] QuickNote:")
        print(colorReset)
        logging.info("    - It is recommended to set the android input type as textNoSuggestions for any sensitive data, if observed.")
        print(colorCyan)
        logging.info("\n[*] Reference:")
        print(colorReset)
        logging.info("    - OWASP MASVS: MSTG-STORAGE-5 | CWE-524: Use of Cache Containing Sensitive Information")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2-data_storage_and_privacy_requirements")
        ...
        if sources_file.endswith(".xml"):
            try:
                ...
                if ":inputType=" in cmd_and_pkg_inputType_output:
                    ...
                    logging.info(sources_file)
                    print(colorReset)
                    logging.info(cmd_and_pkg_inputType_output)
                    countInputType += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countInputType == 0:
        ...
        logging.info("[!] QuickNote:")
        print(colorReset)
        logging.info('    - It is recommended not to disclose any sensitive data such as password, card details, etc. in the clear-text format via User Interface. Make sure that the application is masking sensitive user input by using the inputType="textPassword" attribute. It is useful to mitigate risks such as shoulder surfing.')
        print(colorCyan)
        logging.info("\n[*] Reference:")
        print(colorReset)
        logging.info("    - OWASP MASVS: MSTG-STORAGE-7 | CWE-359: Exposure of Private Personal Information to an Unauthorized Actor")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2-data_storage_and_privacy_requirements")
    ...
    if countInputType > 0:
        ...
        logging.info("[!] QuickNote:")
        print(colorReset)
        logging.info('    - It seems that the application has implemented inputType="textPassword" attribute to hide the certain information, if observed. Make sure that the application is not disclosing any sensitive data such as password, card details, etc. in the clear-text format via User Interface.')
        print(colorCyan)
        logging.info("\n[*] Reference:")
        print(colorReset)
        logging.info("    - OWASP MASVS: MSTG-STORAGE-7 | CWE-359: Exposure of Private Personal Information to an Unauthorized Actor")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "FLAG_SECURE" in cmd_and_pkg_screenShots:
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_screenShots)
                    countScreenShots += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countScreenShots >= 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended to set the FLAG_SECURE option to protect from Auto-Generated Screenshots issue.")
        logging.info("[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-STORAGE-9 | CWE-200: Exposure of Sensitive Information to an Unauthorized Actor")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2-data_storage_and_privacy_requirements")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if ".flush(" in cmd_and_pkg_flushMem:
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_flushMem)
                    countFlushMem += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countFlushMem > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that the sensitive data should be flushed appropriately after its usage.")
        logging.info("[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-STORAGE-10 | CWE-316: Cleartext Storage of Sensitive Information in Memory")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2-data_storage_and_privacy_requirements")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "setPrimaryClip" in cmd_and_pkg_clipCopy or "OnPrimaryClipChangedListener" in cmd_and_pkg_clipCopy:
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_clipCopy)
                    countClipCopy += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countClipCopy > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that any sensitive data should not be copied to the clipboard.")
        logging.info("[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-STORAGE-10 | CWE-316: Cleartext Storage of Sensitive Information in Memory")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2/data_storage_and_privacy_requirements")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if any(keyword in cmd_and_pkg_hardcodeInfo for keyword in ["password", "key", "token", "username", "url", "database", "secret", "bearer"]):
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_hardcodeInfo)
                    countHardInfo += 1
                ...
                if "@" in cmd_and_pkg_hardcodeEmail:
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_hardcodeEmail)
                    countHardInfo += 1
                ...
                if any(ip in cmd_and_pkg_hardcodePrivIP for ip in ["192", "172", "10"]):
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_hardcodePrivIP)
                    countHardInfo += 1
                ...
                if any(url in cmd_and_pkg_cloudURLs for url in ["amazonaws.com", "core.windows.net", "googleapis.com"]):
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_cloudURLs)
                    countHardInfo += 1
                ...
                if "BEGIN" in cmd_and_pkg_begin:
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_begin)
                    countHardInfo += 1
    ...
    if countHardInfo > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that the hard-coded sensitive data (such as Private IPs/E-mails, User/DB details, etc.) should not be stored unless secured specifically.")
        logging.info("[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-STORAGE-14 | CWE-312: Cleartext Storage of Sensitive Information")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2/data_storage_and_privacy_requirements")
        ...
        if sources_file.endswith(".xml"):
            try:
                ...
                if any(keyword in cmd_and_pkg_hardcodedKeys for keyword in ["_key", "_secret", "_token", "_client_id", "_api", "_debug", "_prod", "_stage"]):
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_hardcodedKeys)
    ...
    if countHardcodedKeys > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that the hard-coded keys/tokens/secrets should not be stored unless secured specifically.")
        logging.info("[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-STORAGE-14 | CWE-312: Cleartext Storage of Sensitive Information")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x07-v2/data_storage_and_privacy_requirements")
    ...
    logging.info("\n")
    logging.info(f"{colorBlueBold}[+] Hunting begins based on \"V5: Network Communication Requirements\"{colorReset}")
    logging.info("[+] ----------------------------------------------------------------")

    # Network Security Configuration file check
    logging.info(f"{colorPurple}\n==>> The presence of the Network Security Configuration file...{colorReset}")
    if nwSecConf_final == "`":
    ...
    try:
        ...
        logging.info(f"{colorCyan}\n[!] QuickNote:{colorReset}")
        logging.info("    - It is recommended to configure the Network Security Configuration file (such as network_security_config.xml) as it does not exist. "
        ...
        logging.info("    - OWASP MASVS: MSTG-NETWORK-1 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x10-v5-network_communication_requirements")
    else:
        logging.info(f"{colorCyan}\n[+] QuickNote:{colorReset}")
        logging.info(f"    - It has been observed that Network Security Configuration file is present at:\n      {net_sec_conf_file}")
        logging.info(f"{colorCyan}\n[*] Reference:{colorReset}")
        logging.info("    - OWASP MASVS: MSTG-NETWORK-1 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x10-v5-network_communication_requirements")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "HttpURLConnection" in cmd_and_pkg_unencryptedProtocol_output or "getInsecure" in cmd_and_pkg_unencryptedProtocol_output:
                logging.info(f"{colorBrown}{sources_file}{colorReset}")
                logging.info(cmd_and_pkg_unencryptedProtocol_output)
                countHTTP += 1

    if countHTTP > 0:
        logging.info(f"{colorCyan}\n[!] QuickNote:{colorReset}")
        logging.info("    - It is recommended not to use any unencrypted transmission mechanisms for sensitive data. "
        ...
        logging.info(f"{colorCyan}\n[*] Reference:{colorReset}")
        logging.info("    - OWASP MASVS: MSTG-NETWORK-1 | CWE-319: Cleartext Transmission of Sensitive Information")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x10-v5-network_communication_requirements")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "tls" in cmd_and_pkg_weakTLSProtocol_output or "SSL" in cmd_and_pkg_weakTLSProtocol_output:
                logging.info(f"{colorBrown}{sources_file}{colorReset}")
                logging.info(cmd_and_pkg_weakTLSProtocol_output)
                countWeakTLS += 1

    if countWeakTLS > 0:
        logging.info(f"{colorCyan}\n[!] QuickNote:{colorReset}")
        logging.info("    - It is recommended to enforce TLS 1.2 as the minimum protocol version. "
                    "Please note that, Failure to do so could open the door to downgrade attacks such as DROWN/POODLE/BEAST etc.")
        logging.info(f"{colorCyan}\n[*] Reference:{colorReset}")
        logging.info("    - OWASP MASVS: MSTG-NETWORK-2 | CWE-326: Inadequate Encryption Strength")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x10-v5-network_communication_requirements")
    ...
    for sources_file in files_res:
        if sources_file.endswith(".xml"):
            ...
            if "android:usesCleartextTraffic" in cmd_and_pkg_cleartextTraffic_output or "cleartextTrafficPermitted" in cmd_and_pkg_cleartextTraffic_output:
                logging.info(f"{colorBrown}{sources_file}{colorReset}")
                logging.info(cmd_and_pkg_cleartextTraffic_output)
                countClearTraffic += 1

    if countClearTraffic > 0:
        logging.info(f"{colorCyan}\n[!] QuickNote:{colorReset}")
        logging.info("    - It is recommended to set android:usesCleartextTraffic or cleartextTrafficPermitted to false. "
                    "Please note that, Sensitive information should be sent over secure channels only.")
        logging.info(f"{colorCyan}\n[*] Reference:{colorReset}")
        logging.info("    - OWASP MASVS: MSTG-NETWORK-2 | CWE-319: Cleartext Transmission of Sensitive Information")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x10-v5-network_communication_requirements")
    ...
    for sources_file in files:
        if pathlib.Path(sources_file).suffix == ".java":
            ...
            if "setHostnameVerifier(" in cmd_hostname_verifier_output or "setDefaultHostnameVerifier(" in cmd_hostname_verifier_output or "NullHostnameVerifier" in cmd_hostname_verifier_output or "ALLOW_ALL_HOSTNAME_VERIFIER" in cmd_hostname_verifier_output or "AllowAllHostnameVerifier" in cmd_hostname_verifier_output or "NO_VERIFY" in cmd_hostname_verifier_output or "verify(String" in cmd_hostname_verifier_output:
                logging.info(sources_file)
                logging.info(cmd_hostname_verifier_output)
                count_host_verf += 1

    if count_host_verf > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended not to set ALLOW_ALL_HOSTNAME_VERIFIER or NO_VERIFY, if observed. Please note that, If class always returns true; upon verify() method, the possibility of MITM attacks increases. The application should always verify a hostname before setting up a trusted connection.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-NETWORK-3 | CWE-297: Improper Validation of Certificate with Host Mismatch")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x10-v5-network_communication_requirements")
    ...
    for sources_file in files_res:
        if pathlib.Path(sources_file).suffix in [".cer", ".pem", ".cert", ".crt", ".pub", ".key", ".pfx", ".p12", ".der", ".jks", ".bks"]:
            ...
            count_cert += 1
    if count_cert > 0:
        logging.info(f"{colorCyan}[!] QuickNote:{color_reset}")
        logging.info("    - Hard-coded Certificates/Key/Keystore files have been identified, if observed. Please note that, Attacker may bypass SSL Pinning by adding their proxy's certificate to the trusted keystore with the tool such as keytool.")
        logging.info(f"{colorCyan}\n[*] Reference:{color_reset}")
        logging.info("    - OWASP MASVS: MSTG-NETWORK-4 | CWE-295: Improper Certificate Validation")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x10-v5-network_communication_requirements")
        ...
        if sources_file.endswith('.xml'):
            ...
            if result.returncode == 0:
                ...
                if "<pin" in cmd_and_pkg_cert_pinning_output:
                    logging.info(f"{colorBrown}{sources_file}{color_reset}")
                if any(tag in cmd_and_pkg_cert_pinning_output for tag in ["<pin", "<domain", "<base"]):
                    logging.info(cmd_and_pkg_cert_pinning_output)
                    count_cert_pinning += 1

    if count_cert_pinning > 0:
        logging.info(f"{colorCyan}[!] QuickNote:{color_reset}")
        logging.info("    - It is recommended to appropriately set the certificate pinning in the Network Security Configuration file, if observed. Please note that, The expiration time and backup pins should be set.")
        logging.info(f"{color_cyan}\n[*] Reference:{color_reset}")
        logging.info("    - OWASP MASVS: MSTG-NETWORK-4 | CWE-295: Improper Certificate Validation")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x10-v5-network_communication_requirements")
        ...
        if sources_file.endswith('.xml'):
            ...
            if result.returncode == 0:
                ...
                if any(tag in cmd_and_pkg_trust_anchors_output for tag in ["<certificates", "<domain", "<base"]):
                    logging.info(cmd_and_pkg_trust_anchors_output)
                    count_trust_anchors += 1

    if count_trust_anchors > 0:
        logging.info(f"{colorCyan}[!] QuickNote:{color_reset}")
        logging.info("    - It is recommended that custom Trust Anchors such as <certificates src=user should be avoided, if observed. The <pin> should be set appropriately if it cannot be avoided. Please note that, If the app will trust user-supplied CAs by using a custom Network Security Configuration with a custom trust anchor, the possibility of MITM attacks increases.")
        logging.info(f"{colorCyan}\n[*] Reference:{color_reset}")
        logging.info("    - OWASP MASVS: MSTG-NETWORK-4 | CWE-295: Improper Certificate Validation")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x10-v5-network_communication_requirements")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "ProviderInstaller" in cmd_and_pkg_ProviderInstaller_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_ProviderInstaller_output)
                countProInst += 1

    if countProInst == 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that applications based on the Android SDK should depend on GooglePlayServices, if not observed. Please note that, The ProviderInstaller class is called with either installIfNeeded or installIfNeededAsync to prevent SSL exploits.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-NETWORK-6 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x10-v5-network_communication_requirements")

    if countProInst > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It seems that the ProviderInstaller class is called with either installIfNeeded or installIfNeededAsync to prevent SSL exploits as Android relies on a security provider which comes with the device, if observed.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-NETWORK-6 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x10-v5-network_communication_requirements")
    ...
    logging.info("\n")
    logging.info('[+] Hunting begins based on "V6: Platform Interaction Requirements"')
    logging.info("[+] ---------------------------------------------------------------")
    ...
    for sources_file in files_res:
        if sources_file.endswith(".xml"):
            ...
            if "permission" in cmd_and_pkg_permission_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_permission_output)
                countPerm += 1

    if countPerm > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that the appropriate protectionLevel should be configured in the Permission declaration, if observed. Please note that, Dangerous permissions involve the user’s privacy.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-1 | CWE-276: Incorrect Default Permissions")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files_res:
        if sources_file.endswith(".xml"):
            ...
            if any(perm in cmd_and_pkg_deprecatedPerm_output for perm in ["BIND_", "GET_TASKS", "PERSISTENT_ACTIVITY", "PROCESS_OUTGOING_CALLS", "READ_INPUT_STATE", "RESTART_PACKAGES", "SET_PREFERRED_APPLICATIONS", "SMS_FINANCIAL_TRANSACTIONS", "USE_FINGERPRINT", "UNINSTALL_SHORTCUT"]):
                logging.info(sources_file)
                logging.info(cmd_and_pkg_deprecatedPerm_output)
    ...
    if countDeprecatedPerm > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that the application should not use the Deprecated or Unsupportive permissions, if observed.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-1 | CWE-276: Incorrect Default Permissions")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "checkCallingOrSelfPermission" in cmd_and_pkg_custPerm_output or "checkSelfPermission" in cmd_and_pkg_custPerm_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_custPerm_output)
                countCustPerm += 1

    if countCustPerm > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that Custom Permissions should be used appropriately, if observed. Please note that, The permissions provided programmatically are enforced in the manifest file, as those are more error-prone and can be bypassed more easily with, e.g., runtime instrumentation.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-1 | CWE-276: Incorrect Default Permissions")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if ".rawQuery(" in cmd_and_pkg_sqli_output or ".execSQL(" in cmd_and_pkg_sqli_output or ".appendWhere(" in cmd_and_pkg_sqli_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_sqli_output)
                countSqli += 1

    if countSqli > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that Prepared Statements are used or methods have been used securely to perform any sensitive tasks related to the databases, if observed.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-2 | CWE-89: Improper Neutralization of Special Elements used in an SQL Command ('SQL Injection')")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "javascript" in cmd_and_pkg_xss_output or "evaluateJavascript" in cmd_and_pkg_xss_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_xss_output)
                countXSS += 1

    if countXSS > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that an appropriate encoding is applied to escape characters, such as HTML entity encoding, if observed.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-2 | CWE-79: Improper Neutralization of Input During Web Page Generation ('Cross-site Scripting')")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "getRuntime" in cmd_and_pkg_RCE_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_RCE_output)
                countRCE += 1

    if countRCE > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended not to execute the commands directly on the Operating System or to never use calls to native commands, if observed.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-2 | CWE-78: Improper Neutralization of Special Elements used in an OS Command ('OS Command Injection')")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "PreferenceActivity" in cmd_and_pkg_prefActivity_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_prefActivity_output)
                countPrefAct += 1

    if countPrefAct > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended to implement isValidFragment method or update the android:targetSdkVersion to 19 or higher, if observed. Please note that, With this vulnerability, an attacker can call fragments inside the target application or run the code present in other classes' constructors.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-2 | CWE-470: Use of Externally-Controlled Input to Select Classes or Code ('Unsafe Reflection')")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files_res:
        if sources_file.endswith(".xml"):
            ...
            if "EnableSafeBrowsing" in cmd_and_pkg_EnableSafeBrowsing_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_EnableSafeBrowsing_output)
                countSafeBrow += 1

    if countSafeBrow > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that EnableSafeBrowsing should be configured to true, if observed.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-2 | CWE-940: Improper Verification of Source of a Communication Channel")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    if countPrefAct > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended to implement isValidFragment method or update the android:targetSdkVersion to 19 or higher, if observed.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-2 | CWE-470: Use of Externally-Controlled Input to Select Classes or Code ('Unsafe Reflection')")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    import os
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "shouldOverrideUrlLoading" in cmd_and_pkg_urlLoading_output or "shouldInterceptRequest" in cmd_and_pkg_urlLoading_output:
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_urlLoading_output)
                    countUrlLoad += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countUrlLoad > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended to implement custom URL handlers securely, if observed. Please note that, Even if the attacker cannot bypass the checks on loading arbitrary URLs/domains, they may still be able to try to exploit the handlers.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-2 | CWE-939: Improper Authorization in Handler for Custom URL Scheme")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
        ...
        if sources_file.endswith(".xml"):
            try:
                ...
                if "<intent-filter" in cmd_and_pkg_custUrlSchemes_output:
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_custUrlSchemes_output)
                    countCustUrlSch += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countCustUrlSch > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that custom URL schemes should be configured with android:autoVerify=true, if observed.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-3 | CWE-927: Use of Implicit Intent for Sensitive Communication")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "sendBroadcast(" in cmd_and_pkg_impliIntBroad_output or "sendOrderedBroadcast(" in cmd_and_pkg_impliIntBroad_output or "sendStickyBroadcast(" in cmd_and_pkg_impliIntBroad_output:
                    logging.info(sources_file)
                    logging.info(cmd_and_pkg_impliIntBroad_output)
        ...
        if sources_file.endswith(".java"):
            ...
            if "setJavaScriptEnabled" in cmd_and_pkg_setJavaScriptEnabled_output:
                ...
                if "setJavaScriptEnabled" in cmd_and_pkg_setJavaScriptEnabled_output or "WebView" in cmd_and_pkg_setJavaScriptEnabled_output:
                    logging.info(cmd_and_pkg_setJavaScriptEnabled_output)
                    countSetJavScr += 1

    if countSetJavScr > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended to implement JavaScript execution in WebViews securely, if observed. Please note that, depending on the permissions of the application, it may allow an attacker to interact with the different functionalities of the device.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-5 | CWE-749: Exposed Dangerous Method or Function")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if ".loadUrl" in cmd_and_pkg_loadUrl_output or ".loadDataWithBaseURL" in cmd_and_pkg_loadUrl_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_loadUrl_output)
                countLoadURL += 1

    if countLoadURL > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended to implement Remote/Local URL load in WebViews securely, if observed.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-6 | CWE-940: Improper Verification of Source of a Communication Channel")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "://" in cmd_and_pkg_extLinks_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_extLinks_output)
                countExtLink += 1
                countExtLink2 += cmd_and_pkg_extLinks_output.count("\n")

    if countExtLink > 0:
        logging.info(f"[+] Total file sources are: {countExtLink} & its total instances are: {countExtLink2}\n")
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that external/hard-coded links have been used wisely across the application, if observed.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-6 | CWE-200: Exposure of Sensitive Information to an Unauthorized Actor")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            "setAllowContentAccess" in cmd_and_pkg_fileAccessPerm_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_fileAccessPerm_output)
                countFileAccPerm += 1
    if countFileAccPerm > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended to set Resource Access permissions as false, if observed. Please note that, those functions are quite dangerous as it allows Webview to read all the files that the application has access to.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-6 | CWE-749: Exposed Dangerous Method or Function")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "setWebContentsDebuggingEnabled" in cmd_and_pkg_webConDebug_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_webConDebug_output)
                countWebConDebug += 1
    if countWebConDebug > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended to disable setWebContentsDebuggingEnabled flag, if observed. Please note that, Remote WebView debugging can allow attackers to steal or corrupt the contents of WebViews loaded with web contents (HTML/CSS/JavaScript).")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-6 | CWE-215: Insertion of Sensitive Information Into Debugging Code")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "addJavascriptInterface" in cmd_and_pkg_addJavascriptInterface_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_addJavascriptInterface_output)
                countJavInt += 1
    if countJavInt > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that only JavaScript provided with the APK should be allowed to use the bridges and no JavaScript should be loaded from remote endpoints, if observed. Please note that, this present a potential security risk if any sensitive data is being exposed through those interfaces.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-7 | CWE-749: Exposed Dangerous Method or Function")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "getSerializable" in cmd_and_pkg_serializable_output or "Gson" in cmd_and_pkg_serializable_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_serializable_output)
                countSerialize += 1
    if countSerialize > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended to use Serializable only when the serialized classes are stable, if observed. Reflection-based persistence should be avoided as the attacker might be able to manipulate it to execute business logic.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-8 | CWE-502: Deserialization of Untrusted Data")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            "clearFormData" in cmd_and_pkg_webViewClean_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_webViewClean_output)
                countWebViewCleanUp += 1
    if countWebViewCleanUp == 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended to clear the WebView resources when the application accesses any sensitive data within that, which may include any files stored locally, the RAM cache, and any loaded JavaScript. Please note that, this present a potential security risk if any sensitive data is being exposed.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS: MSTG-PLATFORM-10 | CWE-200: Exposure of Sensitive Information to an Unauthorized Actor")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    if countWebViewCleanUp > 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It seems that the application clears the data via some mechanism, if observed. Please note that, the application should clear all the WebView resources including any files stored locally, the RAM cache, and any loaded JavaScript when it accesses any sensitive data within a WebView.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS V6: MSTG-PLATFORM-10 | CWE-200: Exposure of Sensitive Information to an Unauthorized Actor")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x11-v6-interaction_with_the_environment")
    ...
    for sources_file in files:
        if sources_file.endswith(".java"):
            ...
            if "AppUpdateManager" in cmd_and_pkg_AppUpdateManager_output:
                logging.info(sources_file)
                logging.info(cmd_and_pkg_AppUpdateManager_output)
                countAppUpManag += 1
    if countAppUpManag >= 0:
        logging.info("[!] QuickNote:")
        logging.info("    - It is recommended that applications should be forced to be updated. If a security update comes in, then AppUpdateType.IMMEDIATE flag should be used in order to make sure that the user cannot go forward with using the app without updating it. Please note that, newer versions of an application will not fix security issues that are living in the backends to which the app communicates.")
        logging.info("\n[*] Reference:")
        logging.info("    - OWASP MASVS V1: MSTG-ARCH-9 | CWE-1277: Firmware Not Updateable")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x06-v1-architecture_design_and_threat_modelling_requireme")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "vnd.android.package-archive" in cmd_and_pkg_app_install_output:
                    logging.info(colorBrown + sources_file + color_reset)
                    if ("setDataAndType(" in cmd_and_pkg_app_install_output or 
                        ...
                        logging.info(cmd_and_pkg_app_install_output)
                        count_app_install += 1
            except subprocess.CalledProcessError:
                pass

    if count_app_install > 0:
        logging.info(colorCyan + "[!] QuickNote:" + color_reset)
        logging.info("    - It is recommended to install the application via Google Play and stop using local APK file installation, if observed. If it cannot be avoided, then make sure that the APK file should be stored in a private folder with no overwrite permission. Please note that, Attacker can install a malicious APK file if he/she can control the public folder or path.")
        logging.info(colorCyan + "\n[*] Reference:" + color_reset)
        logging.info("    - OWASP MASVS V1: MSTG-ARCH-9 | CWE-940: Improper Verification of Source of a Communication Channel")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x06-v1-architecture_design_and_threat_modelling_requireme")
    ...
    try:
        ...
        if cmd_and_pkg_debug_regex_match is None:
            logging.info("    - android:debuggable=\"true\" flag has not been observed in the AndroidManifest.xml file.")
        else:
            logging.info(colorBrown + and_manifest_path + color_reset)
            logging.info(f"    - {cmd_and_pkg_debug_regex_match.group()}")
            logging.info(colorCyan + "\n[!] QuickNote:" + color_reset)
            logging.info("    - It is recommended not to enable the debuggable flag, if observed. Please note that, the enabled setting allows attackers to obtain access to sensitive information, control the application flow, etc.")
            logging.info(colorCyan + "\n[*] Reference:" + color_reset)
            logging.info("    - OWASP MASVS V7: MSTG-CODE-2 | CWE-215: Insertion of Sensitive Information Into Debugging Code")
            logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x12-v7-code_quality_and_build_setting_requirements")
    except subprocess.CalledProcessError:
        pass
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "StrictMode" in cmd_and_pkg_strict_mode_output:
                    logging.info(color_brown + sources_file + color_reset)
                    logging.info(cmd_and_pkg_strict_mode_output)
                    count_strict_mode += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if count_strict_mode > 0:
        logging.info(colorCyan + "[!] QuickNote:" + color_reset)
        logging.info("    - It is recommended that StrictMode should not be enabled in a production application, if observed. Please note that, It is designed for pre-production use only.")
        logging.info(colorCyan + "\n[*] Reference:" + color_reset)
        logging.info("    - OWASP MASVS V7: MSTG-CODE-4 | CWE-749: Exposed Dangerous Method or Function")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x12-v7-code_quality_and_build_setting_requirements")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "Exception" in cmd_and_pkg_Exception_output:
                    logging.info(colorBrown + sources_file + colorReset)
                    logging.info(cmd_and_pkg_Exception_output)
                    countExcepHandl += 1
            except subprocess.CalledProcessError:
                pass

    if countExcepHandl > 0:
        logging.info(colorCyan + "[!] QuickNote:" + colorReset)
        logging.info("    - It is recommended that a well-designed and unified scheme to handle exceptions, if observed. Please note that, The application should not expose any sensitive data while handling exceptions in its UI or log-statements.")
        logging.info(colorCyan + "\n[*] Reference:" + colorReset)
        logging.info("    - OWASP MASVS V7: MSTG-CODE-6 | CWE-755: Improper Handling of Exceptional Conditions")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x12-v7-code_quality_and_build_setting_requirements")
    ...
    if countObfusc == 0:
        logging.info(colorCyan + "[!] QuickNote:" + colorReset)
        logging.info("    - It is recommended that some basic obfuscation should be implemented to the release byte-code, if not observed. Please note that, Code obfuscation in the applications protects against reverse engineering, tampering, or other attacks.")
        logging.info(colorCyan + "\n[*] Reference:" + colorReset)
        logging.info("    - OWASP MASVS V7: MSTG-CODE-9 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x12-v7-code_quality_and_build_setting_requirements")

    if countObfusc > 0:
        logging.info(colorCyan + "[!] QuickNote:" + colorReset)
        logging.info("    - It seems that code obfuscation has been identified. It is recommended to check it out manually as well for better clarity.")
        logging.info(colorCyan + "\n[*] Reference:" + colorReset)
        logging.info("    - OWASP MASVS V7: MSTG-CODE-9 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mobile-security.gitbook.io/masvs/security-requirements/0x12-v7-code_quality_and_build_setting_requirements")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "super" in cmd_and_pkg_rootDetect_output or "bin/" in cmd_and_pkg_rootDetect_output:
                    logging.info(colorBrown + sources_file + colorReset)
                    logging.info(cmd_and_pkg_rootDetect_output)
                    countRootDetect += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countRootDetect == 0:
        logging.info(colorCyan + "[!] QuickNote:" + colorReset)
        logging.info("    - It is recommended to implement root detection mechanisms in the application, if not observed. Please note that, Multiple detection methods should be implemented so that it cannot be bypassed easily.")
        logging.info(colorCyan + "\n[*] Reference:" + colorReset)
        logging.info("    - OWASP MASVS V8: MSTG-RESILIENCE-1 | CWE-250: Execution with Unnecessary Privileges")
        logging.info("    - https://mas.owasp.org/MASVS/")

    if countRootDetect > 0:
        logging.info(colorCyan + "[!] QuickNote:" + colorReset)
        logging.info("    - It seems that root detection mechanism has been implemented. Please note that, Multiple detection methods should be implemented. It is recommended to check it out manually as well for better clarity.")
        logging.info(colorCyan + "\n[*] Reference:" + colorReset)
        logging.info("    - OWASP MASVS V8: MSTG-RESILIENCE-1 | CWE-250: Execution with Unnecessary Privileges")
        logging.info("    - https://mas.owasp.org/MASVS/")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "Debug" in cmd_and_pkg_debugDetect_output:
                    logging.info(colorBrown + sources_file + colorReset)
                    logging.info(cmd_and_pkg_debugDetect_output)
                    countDebugDetect += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countDebugDetect == 0:
        logging.info(colorCyan + "[!] QuickNote:" + colorReset)
        logging.info("    - It is recommended to implement Anti-Debugging detection mechanisms in the application, if not observed. Please note that, Multiple detection methods should be implemented so that it cannot be bypassed easily.")
        logging.info(colorCyan + "\n[*] Reference:" + colorReset)
        logging.info("    - OWASP MASVS V8: MSTG-RESILIENCE-2 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mas.owasp.org/MASVS/")

    if countDebugDetect > 0:
        logging.info(colorCyan + "[!] QuickNote:" + colorReset)
        logging.info("    - It seems that Anti-Debugging detection mechanism has been implemented. Please note that, Multiple detection methods should be implemented. It is recommended to check it out manually as well for better clarity.")
        logging.info(colorCyan + "\n[*] Reference:" + colorReset)
        logging.info("    - OWASP MASVS V8: MSTG-RESILIENCE-2 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mas.owasp.org/MASVS/")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "classes" in cmd_and_pkg_intCheck_output:
                    logging.info(colorBrown + sources_file + colorReset)
                    logging.info(cmd_and_pkg_intCheck_output)
                    countIntCheck += 1
            except subprocess.CalledProcessError:
                pass
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "Build" in cmd_and_pkg_emulatorDetect_output or "genyd" in cmd_and_pkg_emulatorDetect_output:
                    logging.info(colorBrown + sources_file + colorReset)
                    logging.info(cmd_and_pkg_emulatorDetect_output)
    ...
    if countEmulatorDetect == 0:
        logging.info(colorCyan + "[!] QuickNote:" + colorReset)
        logging.info("    - It is recommended to implement Emulator detection mechanisms in the application, if not observed. Please note that, Multiple detection methods should be implemented so that it cannot be bypassed easily.")
        logging.info(colorCyan + "\n[*] Reference:" + colorReset)
        logging.info("    - OWASP MASVS V8: MSTG-RESILIENCE-5 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mas.owasp.org/MASVS/")

    if countEmulatorDetect > 0:
        logging.info(colorCyan + "[!] QuickNote:" + colorReset)
        logging.info("    - It seems that Emulator detection mechanism has been implemented. Please note that, Multiple detection methods should be implemented. It is recommended to check it out manually as well for better clarity.")
        logging.info(colorCyan + "\n[*] Reference:" + colorReset)
        logging.info("    - OWASP MASVS V8: MSTG-RESILIENCE-5 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mas.owasp.org/MASVS/")
        ...
        if sources_file.endswith(".java"):
            try:
                ...
                if "SafetyNetClient" in cmd_and_pkg_defenceMech_output:
                    logging.info(colorBrown + sources_file + colorReset)
                    logging.info(cmd_and_pkg_defenceMech_output)
                    countDefenceMech += 1
            except subprocess.CalledProcessError:
                pass
    ...
    if countDefenceMech == 0:
        logging.info(colorCyan + "[!] QuickNote:" + colorReset)
        logging.info("    - It is recommended to implement various defence mechanisms such as SafetyNet Attestation API, if not observed.")
        logging.info(colorCyan + "\n[*] Reference:" + colorReset)
        logging.info("    - OWASP MASVS V8: MSTG-RESILIENCE-7 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mas.owasp.org/MASVS/")

    if countDefenceMech > 0:
        logging.info(colorCyan + "[!] QuickNote:" + colorReset)
        logging.info("    - It seems that SafetyNet APIs have been implemented as part of the various defensive mechanisms.")
        logging.info(colorCyan + "\n[*] Reference:" + colorReset)
        logging.info("    - OWASP MASVS V8: MSTG-RESILIENCE-7 | CWE-693: Protection Mechanism Failure")
        logging.info("    - https://mas.owasp.org/MASVS/")
...
def main():
    ...
    if first_arg not in ["-p", "-m", "-l"]:
        SecDroid_Intro()
        print("\n[!] Invalid argument. Use -h for help.")
        sys.exit(0)

    if first_arg == "-p" and len(sys.argv) > 2:
        apk_path = sys.argv[2]
        SecDroid_Intro()
        SecDroid_core(apk_path)
        sys.exit(0)

    if first_arg == "-p" and len(sys.argv) > 3 and sys.argv[3] == "-l":
        apk_path = sys.argv[2]
        SecDroid_core_log(apk_path)
        SecDroid_core(apk_path)
        sys.exit(0)

    if first_arg == "-m" and len(sys.argv) > 2:
    ...
    if (first_arg == "-m" and len(sys.argv) > 3 and sys.argv[3] == "-l") or (first_arg == "-l" and len(sys.argv) > 3 and sys.argv[2] == "-m"):
        folder_path = sys.argv[3] if first_arg == "-l" else sys.argv[2]
